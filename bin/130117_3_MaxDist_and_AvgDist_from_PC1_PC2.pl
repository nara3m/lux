...
# Syntax Example
# perl ./bin/130117_3_MaxDist_and_AvgDist_from_PC1_PC2.pl SC2 SC2.pca BY4741_24XXXXBY4741_37XXXXElo1_24XXXXElo1_37XXXXElo2_24XXXXElo2_37XXXXElo3_24XXXXElo3_37
#_______________________________________________________________________________________________

my $InputFolderName = $ARGV[0]; print "\n$InputFolderName\n"; 
my $PCAfilename = $ARGV[1]; print "\n$PCAfilename\n";
my $FileNameList = $ARGV[2]; print "\n$FileNameList\n"; 
...
open PCA, "./$InputFolderName/$PCAfilename" or die "\n\nUnable to open \.\/$InputFolderName\/$PCAfilename file\n";
while (<PCA>){
	push @TempArray1, $_;
}
close PCA;
...
# open OUT1, "> ./$InputFolderName/WithConcMaxdist_from_PC1_PC2.csv";
# open OUT2, "> ./$InputFolderName/WithOutConcMaxdist_from_PC1_PC2.csv";
# open OUT3, "> ./$InputFolderName/WithConcAvgDist_from_PC1_PC2.csv";
open OUT4, "> ./$InputFolderName/LUX_PC1_PC2.csv";
...
for ($Var1 = 1; $Var1 < $NumOfInputFiles; $Var1++){
	...
	for ($Var2 = 0; $Var2 < $Var1; $Var2++){
		...
		my $WOC_Mean = max($WOC_Mean_1, $WOC_Mean_2);print "$File1 $File2 $WOC_Mean_1 $WOC_Mean_2\n";
...
sub RunDist {
	...
	foreach (keys (%Sample1)){ # Keys are Molecule names
		...
		if (defined($XYcors{$p1Name})) {
			...
			my ($xp1, $yp1) = split (" ", $XYcors{$p1Name});
			...
			foreach (keys (%Sample2)){
				...
				if (defined($XYcors{$p2Name})) {
					...
					my ($xp2, $yp2) = split (" ", $XYcors{$p2Name});
			
					# Calculating Minimum Distance WithOut considering Concentrations
					$WithOutConcDist = &CalcDist ($p1Name, $p2Name, $xp1, $xp2, $yp1, $yp2);
...
sub CalcDist {
	...
	my ($p1Name, $p2Name, $xp1, $xp2, $yp1, $yp2) = ($_[0], $_[1], $_[2], $_[3], $_[4], $_[5]);
	my $dist; 
