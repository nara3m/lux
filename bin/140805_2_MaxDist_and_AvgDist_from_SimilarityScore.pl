...
my @TempArray1;
my %MoleculeNames;
my @PairWiseDistancesAs2dArray=();

# Reading CSV File
open CSVfile, "./$InputFolderName/$ARGV[0]" or die "Unable to open CSV file";
$Var1=0;
while (<CSVfile>){
	@TempArray1=();
	chomp($_);
	@TempArray1 = split(",", $_);
	$MoleculeNames{"$TempArray1[0]"} = $Var1;
	for($Var2=1;$Var2<scalar(@TempArray1);$Var2++){
		$PairWiseDistancesAs2dArray[$Var1][$Var2-1]=$TempArray1[$Var2];
	}
	$Var1++;
}
close CSVfile;
...
# Writing Results to Files
#open OUT1, "> ./$InputFolderName/WithConcMaxdist_from_SimilarityScore.csv";
#open OUT2, "> ./$InputFolderName/WithOutConcMaxdist_from_SimilarityScore.csv";
#open OUT3, "> ./$InputFolderName/WithConcAvgDist_from_SimilarityScore.csv";
open OUT4, "> ./$InputFolderName/LUX_SimilarityScore.csv";
...
sub RunDist {
	...
	my @TempArray3 = ();
	push (@TempArray3, values(%Sample1), values(%Sample2));
	my $MaxConc = max(@TempArray3); # print "MaxConc = $MaxConc\n";
	...
	foreach (keys (%Sample1)){ # Keys are Molecule names
		...
		my $p1Name = $_;
		if (defined($MoleculeNames{"$p1Name"})){
			...
			foreach (keys (%Sample2)){
				...
				my $p2Name = $_;
				if (defined($MoleculeNames{"$p2Name"})){
			
					# Calculating Minimum Distance WithOut considering Concentrations
					$WithOutConcDist = &CalcDist ($p1Name, $p2Name);
			...
			$WithConcMinDist = ((($WithOutConcMinDist)+(abs($Sample2{$MoleculeThatGaveMinDist} - $cp1)))/2);
...
sub CalcDist {
	...
	if ($p1Name =~ m/\b$p2Name\b/) { # print "Entered line 169";<stdin>;
		...
		$dist=0;
			...
			if ($MoleculeNames{"$p1Name"} < $MoleculeNames{"$p2Name"}){ # Because Pair wise distances between lipid molecues are saved as lower-triangular-matrix
				$dist = $PairWiseDistancesAs2dArray[$MoleculeNames{"$p2Name"}][$MoleculeNames{"$p1Name"}];
			}
			else {
				$dist = $PairWiseDistancesAs2dArray[$MoleculeNames{"$p1Name"}][$MoleculeNames{"$p2Name"}];
			}
	}
...
__END__
	...
	@TempArray3 = ();
	push (@TempArray3, values(%Sample1), values(%Sample2));
	my $MinConc = min(@TempArray3); # print "MinConc = $MinConc\n";
